...
interface ReplayStore extends SessionState {
  ...
  setReplayBounds: (maxIndex: number) => void;
...
export const useReplayStore = create<ReplayStore>((set, get) => ({
  ...
  setReplayBounds: (maxIndex) =>
    set((state) => {
      const boundedMax = Math.max(0, maxIndex);
      return { maxReplayIndex: boundedMax, replayIndex: Math.min(state.replayIndex, boundedMax) };
    }),
