...
export function DrawingLayer({ chart, series }: { chart: IChartApi | null; series: ISeriesApi<'Candlestick'> | null }) {
  ...
  const hitTest = useMemo(
    () => (x: number, y: number): { id: string; target: 'handle' | 'body'; index?: number } | null => {
      ...
      if (previewPoints.length) {
        ...
        renderDrawing(
          ctx,
          {
            ...
            style: { ...defaultStyle, opacity: 0.75 },
            locked: false,
            hidden: false,
        ...
        if (tool?.pointsRequired === 0) setDraft([]);
      }
    };
    const schedule = () => {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      if (dragging || !hostRef.current || !chart || !series) return;
  ...
  const shouldCapture = activeTool !== 'cursor' || Boolean(dragging) || cursorArmed;
