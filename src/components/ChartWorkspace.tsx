...
export function ChartWorkspace() {
  ...
  const resetReplayStart = useReplayStore((s) => s.resetReplayStart);
  ...
  useEffect(() => {
    setReplayBounds(Math.max(0, fullCandles.length - 1));
  }, [fullCandles, setReplayBounds]);

  useEffect(() => {
    if (prevTimeframeRef.current !== timeframe) {
      resetReplayStart(fullCandles);
      prevTimeframeRef.current = timeframe;
    }
  }, [timeframe, fullCandles, resetReplayStart]);
    ...
    <div className="workspace">
      ...
      <div className="workspace-main">
        ...
        <section className="chart-center">
          ...
          <div className="desktop-replay-floating">
            <ReplayControls max={Math.max(0, fullCandles.length - 1)} candlesForFrame={fullCandles} compact />
          </div>
        </section>
      ...
      <nav className="mobile-nav">
        ...
        <button onClick={() => setMobilePanel('replay')}>Replay</button>
